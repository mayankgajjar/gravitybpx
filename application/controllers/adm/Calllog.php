...
class Calllog extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['title'] = 'Admin | Call Log';
        $this->data['pagetitle'] = 'Call Log';
        $this->data['listtitle'] = 'Call Log Listing';
        $this->data['label'] = 'Call Log';
        $this->template->load('admin', 'calllog/index', $this->data);
    }

    public function AdminCalllogJson() {
        $table = 'call_log cl';
        $aColumns = array('cl.unique_id', 'CONCAT(agn.fname," ",agn.lname) AS agent_name', 'age.name AS agency_name','l.member_id AS Lead' , 'cl.number_dialed', 'cl.start_time', 'cl.end_time', 'cl.length_in_sec', 'cl.type', 'r.recording_url');
        $bColumns = array('unique_id',  'agent_name', 'agency_name', 'Lead', 'number_dialed', 'start_time', 'end_time', 'length_in_sec', 'type','recording_url');
        $cColumns = array('agn.fname', 'agn.lname', 'age.name', 'l.member_id', 'cl.number_dialed', 'cl.start_time', 'cl.end_time' , 'cl.length_in_sec' , 'cl.type');
        
        $relation = array(
            "fields" => 'cl.unique_id,CONCAT(agn.fname," ",agn.lname) AS agent_name,age.name AS agency_name,l.member_id AS Lead, cl.number_dialed, cl.start_time, cl.end_time, cl.length_in_sec, cl.type, r.recording_url',
            "JOIN" => array(
                array(
                    "table" => 'agents agn',
