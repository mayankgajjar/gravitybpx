...
class Billing extends CI_Controller {
    ...
    public function __construct() {
        ...
        $stmt_A = "SELECT GROUP_CONCAT(sub.id) AS agents FROM agents sub WHERE sub.agency_id IN ({$agencyList})";
        $agentUsers = $this->db->query($stmt_A)->row_array();
        $ageStr = $agentUsers['agents'];
        $this->agentSql = $ageStr;        
    ...
    public function transaction() {
        ...
        $this->template->load('agency', 'billing/transaction/list', $this->data);
    }
    ...
    public function transactionjson() {
        ...
        $sWhere = ' agent_id IN('.$this->agentSql.')';
        if ($_GET['sSearch'] != "") {
            $sWhere .= " AND(";
    ...
    public function exporttransaction() {
        ...
        $sql = "SELECT concat(a.fname, ' ', a.lname) as `Agent Name`, `name` as `Name`, `stripe_transaction_id` AS `TRANSACTION ID`, `date`, `amount`, `status`, `type` FROM `agent_billing_transactions` LEFT JOIN `agents` `a` ON `agent_billing_transactions`.`agent_id` = `a`.`id` LEFT JOIN `agencies` `age` ON `a`.`agency_id` = `age`.`id` WHERE agent_id IN (".$this->agentSql.") ORDER BY `tran_id`";
