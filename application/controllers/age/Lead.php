...
class Lead extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['title'] = 'Agency | CRM Lead';
        $this->data['maintitle'] = 'Lead';
        $this->data['listtitle'] = "Lead Listing";
        $this->data['label'] = "Lead";
        $this->data['type'] = 'Lead';
        $this->data['addactioncontroller'] = 'age/lead/edit';
        $this->data['importactioncontroller'] = 'age/lead/loadbulk/lead';
        $this->data['exportactioncontroller'] = 'age/lead/export/lead';
        $this->template->load('agency', 'agency/crm/list', $this->data);
    }
    ...
    public function indexjson($type = NULL) {
        ...
        $rResult = $this->leadstore_m->queryForAgency($sWhere, $sOrder, $sLimit);

        $aFilterResult = $this->leadstore_m->queryForAgency($sWhere);
        if ($aFilterResult) {
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                $row[] = '<a title="Edit" href="' . site_url('age/lead/edit/' . lcfirst($type) . '/' . encode_url($aRow['lead_id'])) . '"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="' . site_url('age/lead/delete/' . lcfirst($type) . '/' . encode_url($aRow['lead_id'])) . '"><i class="fa fa-trash" aria-hidden="true"></i></i></a>&nbsp;<a href="javascript:;" title="call" class="btn-xs"><i class="icon-call-out"></i></a>&nbsp;<a href="' . site_url('lead/emailpopup/' . encode_url($aRow['lead_id'])) . '" title="Email Message" data-target="#ajaxemail" data-toggle="modal" title="call" class="btn-xs" ><i class="icon-envelope" title="Email Message"></i></a>&nbsp;<a class="attachment" href="' . site_url('lead/filepopup/' . encode_url($aRow['lead_id'])) . '" data-target="#ajax" data-toggle="modal" title="call" class="btn-xs" title="Document"><i class="fa fa-file"></i></a>';
    ...
    public function edit($type = 'lead', $id = NULL) {
        ...
        $relation = array(
            ...
            "GROUP_BY" => 'agency_id',
            'conditions' => "agency_id IN ({$agencylist})"
        );
        ...
        if ($id) {
            ...
            $this->data['listtitle'] = "Edit " . ucfirst($type);
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data['agency_id'] = decode_url($data['agency_id']);
            ...
            if ($leadId) {
                ...
                redirect('age/lead/edit/' . $type . '/' . $id);
            }
        }
        if ($id) {
            $this->template->load('agency', "agency/crm/edit", $this->data);
        } else {
            $this->template->load('agency', "agency/crm/add", $this->data);
        }
    }
    ...
    public function opportunities() {
        $this->data['title'] = 'Agency | CRM Opportunities';
        $this->data['ckeditor'] = TRUE;
        ...
        $this->data['addactioncontroller'] = 'age/lead/edit/opportunity';
        $this->data['importactioncontroller'] = 'age/lead/loadbulk/opportunity';
        $this->data['exportactioncontroller'] = 'age/lead/export/opportunity';
    ...
    public function clients() {
        $this->data['title'] = 'Agency | CRM Clients';
        ...
        $this->data['addactioncontroller'] = 'age/lead/edit/client';
        $this->data['importactioncontroller'] = 'age/lead/loadbulk/client';
        $this->data['exportactioncontroller'] = 'age/lead/export/client';
    ...
    private function __cancelUrl($type) {
        ...
        switch ($type) {
            case 'lead':
                $url = site_url('age/lead/index');
                break;
            case 'opportunity':
                $url = site_url('age/lead/opportunities');
                break;
            case 'client':
                $url = site_url('age/lead/clients');
                break;
        }
        return $url;
    }
    ...
    public function loadbulk($type = 'lead') {
        $this->data['title'] = 'Agency | CRM ' . ucfirst($type) . ' | Bulk Upload';
        ...
        if ($this->form_validation->run() == TRUE) {
                ...
                while (!feof($handle)) {
                    ...
                    if (strlen($buffer) > 0) {
                        ...
                        if (isset($data['agency']) && strlen($data['agency']) > 0) {
                            ...
                            unset($data['agency']);
                            $data['agency_id'] = $this->session->userdata('agency')->id;
                        }
                        $data['status'] = $status;
                        $data['owner'] = $this->session->userdata('user')->email_id;
                        $data['dispo'] = $dispo;
            ...
            @unlink($file);
            redirect('age/lead/loadbulk/' . $type);
        }
        $this->template->load("agency", "agency/crm/bulk", $this->data);
    }
    ...
    public function addfield($leadId = NULL) {
        ...
        $this->load->view('agency/crm/field', $this->data);
    }
    ...
    public function removefield($leadId = NULL) {
        ...
        $this->load->view('agency/crm/fieldremove', $this->data);
    }
