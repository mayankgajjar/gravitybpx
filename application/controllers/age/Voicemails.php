...
class Voicemails extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['title'] = 'Agency | Voicemail';
        $this->data['pagetitle'] = 'Voicemail';
        $this->data['listtitle'] = 'Voicemail Listing';
        $this->data['label'] = 'Voicemail';
        $this->data ['audiojs'] = TRUE;
        $this->template->load('agency', 'voicemail/index', $this->data);
    }

    public function Agencyvoicemailjson() {
        //$aColumns = array('voicemail_id', 'agent_id', 'from', 'created', 'voicemail_url');
        
        $table = 'voicemails v';
        $aColumns = array('v.voicemail_id', 'CONCAT(agn.fname," ",agn.lname) AS agent_name', 'v.from', 'v.created', 'v.voicemail_url');
        $bColumns = array('voicemail_id',  'agent_name', 'from', 'created', 'voicemail_url');
        $cColumns = array('agn.fname', 'agn.lname', 'v.from', 'v.created');
        
        $relation = array(
            "fields" => 'v.voicemail_id,CONCAT(agn.fname," ",agn.lname) AS agent_name,v.from,v.created,v.voicemail_url',
            "JOIN" => array(
                array(
                    "table" => 'agents agn',
        ...
        $agentlist = getAgentUserIDFromAgncyId();
