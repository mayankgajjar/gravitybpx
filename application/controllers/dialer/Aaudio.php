...
class Aaudio extends CI_Controller{
    ...
    public function index(){
        ...
        $this->template->load('agency', "dialer/agency/audio/list", $this->data);
    }
    public function sound($var){
        ...
        $this->load->view('dialer/agency/audio/sound',$this->data);
    }
    public function uploadfile(){
        if($_FILES['audio']['size'] != 0){
            ...
            if (!$this->upload->do_upload('audio')){
                ...
                $this->audio_m->save(array('audio_name' => $upload_filename,'agency_id' => $this->session->userdata('agency')->id),NULL);
    ...
    public function renamefile(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if(file_exists($oldFile) || $checkData->success){
                ...
                $file = $this->audio_m->get_by(array('audio_name' => $post['master_audiofile'].'.wav','agency_id' => $this->session->userdata('agency')->id  ), TRUE);
                $id = NULL;
//                if($file){
//                    $id = $file->id;
//                }
                $this->audio_m->save(array('audio_name' => $post['new_audiofile'].'.wav','agency_id' => $this->session->userdata('agency')->id ), $id);
