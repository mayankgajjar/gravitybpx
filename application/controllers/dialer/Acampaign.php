...
class Acampaign extends CI_Controller
{
    ...
    public function index(){
        ...
        $this->data['addactioncontroller'] = "dialer/acampaign/campaignedit";
        $this->template->load("agency","dialer/agency/campaign/list",$this->data);
    }
    /**
    ...
    public function indexJson( $join = NULL  ){
        $aColumns = array('campaign_id', 'campaign_name', 'age.name', 'active');
        if($join){
        ...
        $rResult = $this->vcampaigns_m->queryForAgency($sWhere, $sOrder, $sLimit);

        $aFilterResult = $this->vcampaigns_m->queryForAgency($sWhere);
        ...
        $iTotal = count($this->vcampaigns_m->queryForAgency());
        ...
        if($rResult){
            foreach( $rResult as $aRow ){
                ...
                for ( $i=0 ; $i<count($aColumns) ; $i++ ){
                        ...
                        }elseif($aColumns[$i] == 'age.name'){
                            if( $this->session->userdata('agency')->id == $aRow['agency_id'] ){
                                 $row[] = "<span>".$aRow['agency_name']."</span>";
                            }else{
                                $row[] = "<a href='".site_url("agency/agencyedit/".encode_url($aRow['agency_id']))."'>".$aRow['agency_name']."</a>";
                            }
                ...
                if($join){
                    switch($join){
                        case 'status':
                            $row[] = '<a href="'.site_url('dialer/acampaign/statusedit/'.encode_url($aRow['campaign_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>';
                        break;
                        case 'hotkey':
                        ...
                        break;
                        case 'autoalt':
                            $row[] = '<a href="'.site_url('dialer/acampaign/autoaltedit/'.encode_url($aRow['campaign_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>';
                        break;
                        case 'code':
                    ...
                    $row[] = '<a href="'.site_url('dialer/acampaign/campaignedit/'.encode_url($aRow['campaign_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="'.site_url('dialer/acampaign/campaigndelete/'.encode_url($aRow['campaign_id'])).'"><i class="fa fa-trash" aria-hidden="true"></i></i>';
                }
                $output['aaData'][] = $row;
            }
        }else{
    ...
    public function campaignedit($id = NULL){
        ...
        $this->data['ingroups'] = getInboundGroupsForUser($this->session->userdata('agency')->id);
        $rules = $this->vcampaigns_m->rules;

        if($id){
            $id = decode_url($id);
            $this->data['campaign'] = $this->vcampaigns_m->get_by(array('campaign_id' => $id), TRUE);
            count($this->data['campaign']) || $this->data['errors'][] = 'Campaign could not be found.';
        ...
        if( $this->form_validation->run() == TRUE ){
            ...
            redirect('dialer/acampaign/campaignedit/'.  encode_url($camId));
        }
        if($id){
            $this->template->load("agency","dialer/agency/campaign/edit",$this->data);
        }else{
            $this->template->load("agency","dialer/agency/campaign/add",$this->data);
        }
    }
    ...
    public function campaigndelete($id = NULL){
        ...
        redirect('dialer/acampaign/index');
    }
    /**
    ...
    public function statusindex(){
        ...
        $this->template->load("agency","dialer/agency/campaign/status/list",$this->data);
    }
    /**
    ...
    public function statusedit($id = NULL, $statusid = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            redirect('dialer/acampaign/statusedit/'. encode_url($id));
        }

        $this->template->load("agency","dialer/agency/campaign/status/edit",$this->data);
    }
    /**
    ...
    public function autoaltindex(){
        ...
        $this->template->load("agency","dialer/agency/campaign/autoalt/list",$this->data);
    }
    ...
    public function autoaltedit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($campid){
                ...
                redirect('dialer/acampaign/autoaltedit/'.encode_url($id));
            }
        }
        ...
        $this->template->load("agency","dialer/agency/campaign/autoalt/edit",$this->data);
    }
