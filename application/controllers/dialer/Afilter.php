...
class Afilter extends CI_Controller{
    ...
    public function index(){
        ...
        $this->data['addactioncontroller'] = "dialer/afilter/edit";
        $this->template->load("agency","dialer/agency/filter/list",$this->data);
    }
    public function indexjson(){
        $aColumns = array( 'lead_filter_id' , 'lead_filter_name', 'user_group', 'agency_id');
        /*
         * Paging
         */
        $sLimit = "";
        ...
        $rResult = $this->afilter_m->queryForAgency($sWhere, $sOrder, $sLimit);

        $aFilterResult = $this->afilter_m->queryForAgency($sWhere);
        if($aFilterResult){
            ...
            foreach( $rResult as $aRow )
                {
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                        {
                            if($aColumns[$i] == 'agency_id'){
                                $agency = $this->agency_model->getAgencyInfo($aRow['agency_id']);
                                $row[] = $agency->name;
                            }else{
                                $row[] = $aRow[ $aColumns[$i] ];
                            }
                        }
                        $row[] = '<a href="'.site_url('dialer/afilter/edit/'.encode_url($aRow['lead_filter_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="'.site_url('dialer/afilter/delete/'.encode_url($aRow['lead_filter_id'])).'"><i class="fa fa-trash" aria-hidden="true"></i></i>';
    ...
    public function edit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($filterId){
                $aData = array('agency_id' => $this->session->userdata('agency')->id,'vicidial_filter_id' => $filterId, 'filter_type' => $type, 'filter_options' => $filter_options, 'filter_values' => $this->input->post('filter_values'));
                ...
                redirect('dialer/afilter/edit/'.encode_url($filterId));
            }
        }
        $this->template->load('agency', "dialer/agency/filter/edit", $this->data);
    }
