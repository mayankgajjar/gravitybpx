...
class Aphones extends CI_Controller{
    ...
    public function index(){
        ...
        $this->template->load('agency',"dialer/agency/phones/list",$this->data);
    }
    ...
    public function edit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($phoneId){
                ...
                redirect('dialer/aphones/edit/'.encode_url($phoneId));
            }
        }

        $this->template->load("agency","dialer/agency/phones/edit",$this->data);
    }
    ...
    public function agencyindex(){
        ...
        $this->template->load('agency',"dialer/agency/phones/agency/list",$this->data);
    }
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                        {
                            if($aColumns[$i] == 'p.id'){
                                ...
                                $row[] = '<a href="'.site_url('agency/manage_agency/agency_info/'.$aRow['agency_id']).'">'.$aRow['name'].'</a>';
                            }else{
                                $row[] = $aRow[ $aColumns[$i] ];
                            }
                        }
                        $row[] = '<a href="'.site_url('dialer/aphones/edit/'.encode_url($aRow['id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="'.site_url('dialer/aphones/delete/'.encode_url($aRow['id'])).'"><i class="fa fa-trash" aria-hidden="true"></i></i>';
    ...
    public function agentndex(){
        ...
        $this->template->load('agency',"dialer/agency/phones/agent/list",$this->data);
    }
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                        {
                            if($aColumns[$i] == 'p.id'){
                                ...
                                $row[] = '<a href="'.site_url('agency/manage_agent/agent_info/'.$aRow['agent_id']).'">'.$aRow['fname'].' '.$aRow['lname'].'</a>';
                            }else{
                                $row[] = $aRow[ $aColumns[$i] ];
                            }
                        }
