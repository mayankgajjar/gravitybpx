...
class Lists extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['addactioncontroller'] = "dialer/lists/edit";
        $this->template->load('admin', "dialer/admin/lists/list", $this->data);
    }
    public function indexJson() {
        ...
        $rResult = $this->vlists_m->query($sWhere, $sOrder, $sLimit);

        $aFilterResult = $this->vlists_m->query($sWhere);
        if ($aFilterResult) {
            $iFilteredTotal = count($aFilterResult);
        } else {
            $iFilteredTotal = 0;
        }
        $iTotal = count($this->vlists_m->get());
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                for ($i = 0; $i < count($aColumns); $i++) {
                    if($aColumns[$i] == 'leads_count'){
                        ...
                        if($single){
                            ...
                            $row[] = '<a target="_blank" href="'.site_url('admin/manage_agency/agency_info/'.$agency->id).'">'.$agency->name.'</a>';
                        }else{
                            $row[] = '';
                        }
                    }else{
                ...
                $row[] = '<a href="' . site_url('dialer/lists/edit/' . encode_url($aRow['list_id'])) . '"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="' . site_url('dialer/lists/delete/' . encode_url($aRow['list_id'])) . '"><i class="fa fa-trash" aria-hidden="true"></i></i>';
...
//            }
            ...
            if ($listId) {
                ...
                redirect('dialer/lists/edit/' . encode_url($listId));
            }
        }
        $this->template->load('admin', "dialer/admin/lists/edit", $this->data);
    }
    ...
    public function addlead($id = NULL) {
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($lead_id){
                ...
                redirect('dialer/lists/addlead/'.  encode_url($lead_id));
            }
        }

        $this->template->load('admin', "dialer/admin/lead/add", $this->data);
    }
    ...
    public function getcity(){
        ...
        if($post){
            ...
            foreach($cities as $city){
                $selected = '';
                if(ucwords($city->name) == ucwords($post['city'])){
                    $selected = 'selected="selected"';
                }
    ...
    public function getCampaigns(){
        ...
        if( $post && $post['id'] > 0){
            ...
            if(count($campaignId) <= 0){
                ...
                $string = $this->load->view('dialer/admin/lists/campaign',$this->data,TRUE);
                $data['html'] = $string;
                return $this->output
                        ->set_content_type('application/json')
                        ->set_output(json_encode($data));
    ...
    public function loadlead(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            redirect('dialer/lists/loadlead');
        }

        $this->template->load('admin','dialer/admin/lead/loadlead',$this->data);
    }
    ...
    public function dncindex(){
        $this->data['datatable'] = TRUE;
        $this->data['sweetAlert'] = TRUE;
        $this->data['models'] = TRUE;
        $this->data['meta_title'] = "DNC Lists";
        $this->data['title'] = "DNC List";
        $this->data['breadcrumb'] = "DNC";
        $this->data['listtitle'] = "DNC Listing";
        $this->data['addactioncontroller'] = "dialer/lists/dncedit";
        $this->template->load('admin', "dialer/admin/lists/dnc/list", $this->data);
    }
    public function dncindexJson() {
        $aColumns = array('phone_number');
        /*
         * Paging
         */
        $sLimit = "";
        ...
        $rResult = $this->vdnc_m->query($sWhere, $sOrder, $sLimit);

        $aFilterResult = $this->vdnc_m->query($sWhere);
        if ($aFilterResult) {
            $iFilteredTotal = count($aFilterResult);
        } else {
            $iFilteredTotal = 0;
        }
        $iTotal = count($this->vdnc_m->get());
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                $row = array();
                //$row[] = $count++;
                $row[] = '<input type="checkbox" class="checkboxes"  name="id[]" value="' . $aRow['phone_number'] . '"/>';
                for ($i = 0; $i < count($aColumns); $i++) {
                        $row[] = isset($aRow[$aColumns[$i]]) ? $aRow[$aColumns[$i]] : '';
                }
                $row[] = '<a data-toggle="modal" data-target="#ajax" href="' . site_url('dialer/lists/dncedit?' . http_build_query(array('id' => $aRow['phone_number']))) . '"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="' . site_url('dialer/lists/dncdelete?' .http_build_query(array('id' => $aRow['phone_number']))) . '"><i class="fa fa-trash" aria-hidden="true"></i></i>';
                $output['aaData'][] = $row;
            }
        } else {
            $output['aaData'] = array();
    ...
    public function dncdelete($id = NULL) {
        if ($this->input->get('id')) {
            $this->vdnc_m->delete($this->input->get('id'));
            $this->session->set_flashdata('success', 'Phone number deleted successfully from DNC.');
        } else {
            $this->session->set_flashdata('error', 'Phone number doesn\'t exist.');
        }
        redirect('dialer/lists/dncindex');
    }
    public function dncmassaction() {
        $ids = $this->input->post('id');
        if (empty($ids)) {
            $this->session->set_flashdata('error', 'No phone number have been selected.');
            redirect('dialer/lists/dncindex');
        }
        $action = $this->input->post('action');
        switch ($action) {
    ...
    public function dncedit($id = NULL){
        if( $this->input->get('id') ){
            $this->data['number'] = $this->vdnc_m->get_by(array('phone_number' => $this->input->get('id')), TRUE);
        }else{
            $this->data['number'] = new stdClass();
            $this->data['number']->phone_number = '';
        }
        $this->form_validation->set_rules('phone_number','Phone Number','trim|required');
        if( $this->form_validation->run() == TRUE){
            $data = $this->vdnc_m->array_from_post(array(
                'phone_number'
            ));
            $this->vdnc_m->save($data,$id);
            $output['success'] = TRUE;
            return $this->output
                        ->set_content_type('application/json')
                        ->set_output(json_encode($output));
        }
        $this->load->view('dialer/admin/lists/dnc/edit', $this->data);
    }
    public function searchlead(){
        ...
        $this->template->load('admin','dialer/admin/lead/search',$this->data);
    }
    public function searchleadpost(){
		...
		$stmt="SELECT $vicidial_list_fields from $vl_table where vendor_lead_code=" . $this->vicidialdb->db->escape($post['vendor_id']) . "";
               ...
               redirect('dialer/lists/searchlead');
           }
            $stmt.=' limit 0,1000';
            ...
            $this->template->load('admin','dialer/admin/lead/result',$this->data);
        }else{
            $this->session->set_flashdata('error','You must search for something! Go back and search for something');
            redirect('dialer/lists/searchlead');
        }
