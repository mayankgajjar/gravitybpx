...
class Remote extends CI_Controller{
    ...
    public function index(){
        ...
        $this->template->load('admin', "dialer/admin/remote/list", $this->data);
    }
            ...
            foreach( $rResult as $aRow )
                {
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                        ...
                        $row[] = '<a href="'.site_url('dialer/remote/edit/'.encode_url($aRow['remote_agent_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="'.site_url('dialer/remote/delete/'.encode_url($aRow['remote_agent_id'])).'"><i class="fa fa-trash" aria-hidden="true"></i></i>';
    ...
    public function edit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($remoteId){
                ...
                redirect('dialer/remote/edit/'.  encode_url($remoteId));
            }
        }
        $this->template->load('admin', "dialer/admin/remote/edit", $this->data);
    }
    function _unique_remote_id(){
        $id = $this->uri->segment(4);
        $this->vicidialdb->db->where('user_start',$this->input->post('user_start'));
        !$id || $this->vicidialdb->db->where(' user_start!=', $this->input->post('user_start'));
    ...
    public function groupindex(){
        ...
        $this->template->load('admin', "dialer/admin/remote/group/list", $this->data);
    }
            ...
            foreach( $rResult as $aRow )
                {
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                        ...
                        $row[] = '<a href="'.site_url('dialer/remote/groupedit/'.encode_url($aRow['extension_id'])).'"><i class="fa  fa-pencil-square-o" aria-hidden="true"></i></a>&nbsp;&nbsp;<a class="delete" href="'.site_url('dialer/remote/groupdelete/'.encode_url($aRow['extension_id'])).'"><i class="fa fa-trash" aria-hidden="true"></i></i>';
    ...
    public function groupedit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($gId){
                ...
                redirect('dialer/remote/groupedit/'.  encode_url($gId));
            }
        }
        $this->template->load('admin', "dialer/admin/remote/group/edit", $this->data);
    }
    ...
    public function groupdelete($id = NULL){
        if($id){
            ...
            redirect('dialer/remote/groupindex');
        }else{
