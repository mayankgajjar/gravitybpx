...
class Stripe {
     ...
     * @param  string        A free form reference for the charge
     */
    public function main_charge_card($amount, $card, $desc) {
        $params = array(
            'amount' => $amount,
            'currency' => 'usd',
            'card' => $card,
    ...
    /**
     * Get a list of customers cards record
     *
     * @param  int           The number of customers to return, default 10, max 100
     * @param  int           Offset to apply to the list, default 0
     */
    public function customer_list_cards($customerId, $limit = 3) {
        //$params['count'] = $count;
        //$params['offset'] = $offset;
        $params['object'] = 'card';
        $params['limit'] = $limit;
        $vars = http_build_query($params, NULL, '&');
        return $this->_send_request('customers/' . $customerId . '/sources?' . $vars);
    }

    /**
     * add card to customer data
     *
     * @param  int           The number of customers to return, default 10, max 100
     * @param  int           Offset to apply to the list, default 0
     */
    public function customer_create_card($customerId, $source = NULL) {
        $params['source'] = $source;
        return $this->_send_request('customers/' . $customerId . '/sources', $params, 'post');
    }

    /**
     * add card token to add card data
     *
     * @param  int           The number of customers to return, default 10, max 100
     * @param  int           Offset to apply to the list, default 0
     */
    public function create_card_token($card_data = array()) {
        return $this->_send_request('tokens', $card_data, 'post');
    }

    /**
     * update card token to add card data
     *
     * @param  int           The number of customers to return, default 10, max 100
     * @param  int           Offset to apply to the list, default 0
     */
    public function update_card_data($card_id = NULL, $customer_id = NULL, $card_data = array()) {
        return $this->_send_request('customers/' . $customer_id . '/' . 'sources/' . $card_id, $card_data, 'post');
    }

    /**
     * delete card token to add card data
     *
     * @param  int           The number of customers to return, default 10, max 100
     * @param  int           Offset to apply to the list, default 0
     */
    public function delete_card_data($card_id = NULL, $customer_id = NULL, $card_data = array()) {
        return $this->_send_request('customers/' . $customer_id . '/' . 'sources/' . $card_id, $card_data, STRIPE_METHOD_DELETE);
    }

    public function retrive_card($card_id = NULL, $customer_id = NULL) {
        return $this->_send_request('customers/' . $customer_id . '/' . 'sources/' . $card_id, array(), STRIPE_METHOD_POST);
    }
