...
{    
    ...
    public function getAll()
    {
        ...
        {                    
            ...
            $prodcuts = $this->db->query("SELECT products.*,category.category_name,company.company_name,company.company_logo FROM products                
                LEFT JOIN company ON company.id = products.underwriting_company
                LEFT JOIN category ON category.id = products.category_id group by products.id order by products.id DESC");        
    ...
    /*
     * getAllOrderBy is used to retrive all record of products table
     * 
     * return two dimensional array in the form of array
     */
    public function getAllOrderBy($order_column_name,$order_by)
    {
        try
        {                    
            ...
            $prodcuts = $this->db->query("SELECT products.*,category.category_name,company.company_name,company.company_logo FROM products                
                LEFT JOIN company ON company.id = products.underwriting_company
                LEFT JOIN category ON category.id = products.category_id group by products.id ORDER BY $order_column_name $order_by");        
            return $prodcuts->result_array();            
        }
        catch(Exception $e)
        {
    ...
    public function getAllProductByStatus($customActionName)
    {
        try
        {                                
            $prodcuts = $this->db->query("SELECT products.*,company.company_name,company.company_logo FROM products                                
                LEFT JOIN company ON company.id = products.underwriting_company WHERE products.is_active = ".$customActionName." GROUP BY products.id");        
            return $prodcuts->result_array();            
        }
        catch(Exception $e)
        {
    ...
    public function getAllProductByParameters($search_string)
    {
        ...
        {      
            if(isset($search_string) && $search_string != "")
            {
                $prodcuts = $this->db->query("SELECT products.*,category.category_name,company.company_name,company.company_logo FROM products                                
                LEFT JOIN company ON company.id = products.underwriting_company
                LEFT JOIN category ON category.id = products.category_id WHERE ".$search_string." GROUP BY products.id");            
            }   
            else
            {
                $prodcuts = $this->db->query("SELECT products.*,category.category_name,company.company_name,company.company_logo FROM products                                
                LEFT JOIN company ON company.id = products.underwriting_company
                LEFT JOIN category ON category.id = products.category_id GROUP BY products.id");            
            }                           
            return $prodcuts->result_array();            
        }
        catch(Exception $e)
    ...
    {        
        ...
        {     
            ...
            $result = $this->db->query("SELECT products.*,category.category_name,company.company_name,company.company_logo,group_concat(state.name separator  ',') AS product_states_names,country.name AS country_name,country.id AS country_id FROM products
                /*LEFT JOIN product_state ON product_state.product_id = products.id*/
           ...
           $prodcuts = $result->row();                    
           $arrayvalue = array();
           if($prodcuts->state_id != "")
           {        
               foreach (explode(',',$prodcuts->state_id) as $key => $value) 
               {
                  $arrayvalue[] = $this->db->query("SELECT name FROM state WHERE id = $value")->row()->name;
               }               
           }           
           $prodcuts->product_states_names = implode(',',$arrayvalue);            
           return $prodcuts;
        }
        catch(Exception $e)
        {
    ...
    public function delete($id)
    {
        try
        {
            ...
            return $this->db->query("DELETE p FROM products p WHERE p.id=".$id);                    
        }
        catch(Exception $ex)
        {
            return false;
    ...
    {                                
        ...
        {*/
            ...
            {                                
                $prodcuts = $this->db->query("SELECT products.*,company.company_name,company.company_logo FROM products                    
                    JOIN company ON company.id = products.underwriting_company
                    JOIN state ON state.id IN (products.state_id) ".$this->state." WHERE is_active = 1 ".$this->filter_query." group by products.id");                              
                return $prodcuts->result_array();
    ...
    {        
        ...
        {                                
           ...
           if($result->state_id != "")
           {        
               foreach (explode(',',$result->state_id) as $key => $value) 
               {
                  $arrayvalue[] = $this->db->query("SELECT name FROM state WHERE id = $value")->row()->name;
               }
               $result->product_states_names = implode(',',$arrayvalue);
           }
           return $result;
        }
        catch(Exception $e)
