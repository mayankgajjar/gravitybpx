...
class Aregroup_m extends My_Model{
    protected $_table_name     = 'agency_extension_group';
    ...
    public function query($where = '', $order = '', $limit = '') {
        ...
        $sql = "SELECT main.*,amain.id,amain.agency_id,age.name FROM {$this->db->protect_identifiers($_tempTable, TRUE)} AS main LEFT JOIN {$this->db->protect_identifiers($this->_table_name, TRUE)} amain ON amain.vicidial_extension_id = main.extension_id LEFT JOIN {$this->db->protect_identifiers('agencies', TRUE)} age ON amain.agency_id=age.id ";
