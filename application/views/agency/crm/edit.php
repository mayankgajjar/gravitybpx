    ...
    <div class="portlet-title">
        ...
        <div class="actions">
            <?php if ($lead->lead_id != '') : ?>
                <?php if($lead->status != '' && $lead->status == 'Lead') :?>
                    <a href="<?=base_url('age/lead/change_status/Opportunity/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Lead to Opportunities</a>
                    <a href="<?=base_url('age/lead/change_status/Client/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Lead to Client</a>
                <?php elseif($lead->status != '' && $lead->status == 'Opportunity') :?>
                    <a href="<?=base_url('age/lead/change_status/Lead/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Opportunities to Lead</a>
                    <a href="<?=base_url('age/lead/change_status/Client/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Opportunities to Client</a>
                <?php elseif($lead->status != '' && $lead->status == 'Client') :?>
                    <a href="<?=base_url('age/lead/change_status/Lead/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Client to Lead</a>
                    <a href="<?=base_url('age/lead/change_status/Opportunity/'.encode_url($lead->lead_id))?>" class="btn green"><i class="fa fa-refresh"></i> Convert Client to Opportunities</a>
                <?php endif; ?>
            <?php endif; ?>            
        </div>
    </div>    
        <div class="portlet-body">
            <div class="tabbable-line">
                <?php $this->load->view('agency/crm/tabs') ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="agency_id" class="form-control" class="agency_id" onchange="javascript:selectAgent(this.value)">
                                <option value="<?php echo encode_url($this->session->userdata('agency')->id) ?>" ><?php echo "Please Select Agency"; ?></option>
                                <?php echo printTreeWithEncrypt($tree,0, null, isset($lead->agency_id) ? $lead->agency_id : '' ); ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-4">
                                    <select name="dispo" class="form-control" id="dispo">
                                        <option value="QUOTED"><?php echo 'QUOTED' ?></option>                            
                                        <option value="CALLBACK"><?php echo 'CALLBACK' ?></option>
                                    </select>
                                </div>
                                 <script>
                                    $("#dispo").val("<?=$lead->dispo?>");
                                </script>
                            </div>
                        <?php endif; ?>    
                        <div class="form-group">
                    ...
                    <?php if ($lead->lead_id > 0): ?>
                        <div class="portlet-title">
                            ...
                            <div class="actions" style="display: inline-block;float: right;">
                                <a href="<?php echo site_url('age/lead/addfield/' . encode_url($lead->lead_id)) ?>" class="btn green" type="button" data-target="#ajax" data-toggle="modal"><?php echo 'Add Field' ?></a>
                                <a href="<?php echo site_url('age/lead/removefield/' . encode_url($lead->lead_id)) ?>" class="btn green" type="button" data-target="#ajaxremove" data-toggle="modal"><?php echo 'Remove Field' ?></a>            
                            </div>
...
<script type="text/javascript">
    ...
    function renderCustomFields(){
        jQuery.ajax({
            url : '<?php echo site_url("age/lead/renderfields/". encode_url($lead->lead_id)) ?>',
            method : 'post',
            dataType : 'json',
            success : function(result){
