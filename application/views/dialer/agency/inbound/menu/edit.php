...
<div class="portlet light bordered">
    ...
    <div class="portlet-body">
        <form id="options" name="options" id="groupForm" action="<?php echo 'dialer/ainbound/savemenuoption' ?>" method="post" class="form-inline" >
...
jQuery(function(){
    selectTrackingGroup('<?php echo $this->session->userdata('agency')->id ?>','<?php echo $menu->tracking_group ?>');
...
function selectGroup(agency_id, groupId){
    ...
    selectTrackingGroup(agency_id,groupId);
}
...
function selectTrackingGroup(agencyId, groupId){
    groupId = '<?php echo $menu->tracking_group ?>';
    jQuery.ajax({
       url : '<?php echo site_url('dialer/ajax/gettrackingingroups') ?>',
       method : 'POST',
       dataType : 'json',
