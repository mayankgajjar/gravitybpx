        ...
        <form id="teamform" name="teamform" method="post">
            ...
            <div class="form-body">
                    ...
                    <div class="col-md-4">
                         <div class="form-group">
                            ...
                            <div class="input-group input-large date-picker input-daterange" data-date="10/11/2012" data-date-format="mm/dd/yyyy">
                                <input type="text" class="form-control" name="query_date_D" value="<?php echo isset($postData['query_date_D']) ? $postData['query_date_D'] :  date('m/d/Y') ?>">
                                <span class="input-group-addon"> to </span>
                                <input type="text" class="form-control" name="end_date_D" value="<?php echo isset($postData['end_date_D']) ? $postData['end_date_D'] : date('m/d/Y') ?>">
                            </div>
                         </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="input-group input-large time-picker" data-date="10/11/2012" data-date-format="mm/dd/yyyy">
                                <input type="text" class="form-control" name="end_date_T" value="<?php echo isset($postData['query_date_T']) ? $postData['query_date_T'] :  '00:00:00' ?>">
                                <span class="input-group-addon"> to </span>
                                <input type="text" class="form-control" name="end_date_T" value="<?php echo isset($postData['end_date_T']) ? $postData['end_date_T'] : '23:59:59' ?>">
                            </div>
                         </div>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label><?php echo "Show additional statuses" ?></label>
                            <?php $callstatusArray = isset($postData['call_status']) ? $postData['call_status'] : array('--NONE--') ?>
                            <?php $selected = ''; ?>
                            <select name="call_status[]" class="form-control" id="call_status" multiple="">
                                <?php if(in_array('--NONE--', $callstatusArray)): ?>
                                    <?php $selected = 'selected="selected"'; ?>
                                <?php endif; ?>
                                <option value="--NONE--" <?php echo $selected ?>><?php echo "NO ADDITIONAL STATUSES" ?></option>
                                <?php if(count($callStatuses) > 0): ?>
                                    <?php foreach($callStatuses as $callStatus): ?>
                                        <?php $callStatus = array_values($callStatus);  ?>
                                        <?php $selected = '' ?>
                                        <?php if(in_array($callStatus[0], $callstatusArray )): ?>
                                            <?php $selected = 'selected="selected"'; ?>
                                        <?php endif; ?>
                                        <option value="<?php echo $callStatus[0] ?>" <?php echo $selected; ?>><?php echo $callStatus[0].' - '.$callStatus[1] ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
...
function selectOther(agency){
    ...
    var userGroup = '<?php echo isset($postData['user_group']) ? json_encode($postData['user_group']) : '' ?>';
    jQuery.ajax({
        url: '<?php echo site_url('dialer/report/getother') ?>',
        method: 'POST',
        data: { id : agency, usergroup: userGroup},
        dataType:'JSON',
